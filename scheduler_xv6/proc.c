...
//-----------------------------------------------------------------------------------
// figure out how to generate random numbers in the kernel
// how to generate a simple pseudo-random number generator
...
// random choose a winner ticket between 0 and total_tickets
// https://stackoverflow.com/questions/2509679/how-to-generate-a-random-integer-number-from-within-a-range
// Assume:  0 <= total_tickets <= RAND_MAX
// returns in the closed interval [0, total_tickets]
long
get_winner(int total_tickets)
{
  unsigned long
  // total_tickets <= RAND_MAX < ULONG_MAX
  num_bins = (unsigned long) total_tickets + 1,
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // count: track to help find the winner
    int count = 0;
    int total_tickets = 0;
    ...
    long winner = get_winner(total_tickets);
...
// This routine returns some information about all running processes, including
// how many times each has been chosen to run and the process ID of each.
int
